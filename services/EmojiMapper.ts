...
export class EmojiMap {
  ...
  async loadEmojiMap(): Promise<Record<string, string>> {
    try {
      ...
      const file = await this.app.vault.adapter.read(`${this.manifestDir}/data/emoji-map.json`);
      return JSON.parse(file);
    } catch (e: any) {
      if (e instanceof SyntaxError) {
        console.warn("data/emoji-map.json contains invalid JSON. Please fix or regenerate the file.", e);
      } else {
        console.warn("Could not load data/emoji-map.json (file missing or unreadable).", e);
      }
      return {};
    }
  }
  ...
  async buildMap(): Promise<{added: number, total: number}> {
    ...
    const mapPath = `${this.manifestDir}/data/emoji-map.json`;
    ...
    // Note: Pruning of deleted entries is now handled by EmojiCooker using centralized validation
